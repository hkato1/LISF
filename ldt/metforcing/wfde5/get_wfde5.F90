...
!EOP
  integer           :: order
  integer           :: ferror
  character*100,dimension(8)     :: fname
...
! _________________________________________________________
  ...
  if( LDT_rc%nts(n).gt.3600 ) then   ! > 1-hr timestep
     write(LDT_logunit,*) '[ERR] When running LDT with WFDE5, the clock '
...
!=== the following looks back 10 days, at the same hour to fill data gaps.
     ...
     do 
        ...
        write(unit=LDT_logunit,fmt=*)'[INFO] getting file.. ',trim(fname(1))
        ...
        if(try.gt.11)then
           write(LDT_logunit,*)'[ERR] WFDE5 data gap exceeds 10 days on file 1'
           call LDT_endrun()
        endif
     enddo
!=== end of data search
...
!=== the following looks back 10 days, at the same hour to fill data gaps.
     ...
     !- Obtaining WFDE5 File:
        ...
        if(try.gt.11)then
           write(LDT_logunit,*)'[ERR] WFDE5 data gap exceeds 10 days on file 2'
           call LDT_endrun()
        endif
     enddo
  endif 
...
!EOP
  ...
  if ( presrc.eq."GPCC" ) then
    ...
    fname(6) = trim(wfde5dir)//'/'//trim(cyear)//'/Snowff_WFDE5_CRU+GPCC_'//trim(cyear)//trim(cmonth)//'_v2.0.nc'
  else
