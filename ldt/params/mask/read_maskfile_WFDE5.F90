...
!
! SUBROUTINE: read_maskfile_WFDE5
!
! REVISION HISTORY:
! 29 Mar 2022  Initial version....Hiroko Beaudoing
!
! DESCRIPTION:
! Source code for reading and interpolating land mask data in netCDF format
! produced from WFDE5 elevation/land data.  
! The routines are adopted from the read_maskfile_UKMO_CAP_Netcdf subroutine.
!
!------------------------------------------------------------------------------

#include "LDT_misc.h"

subroutine read_maskfile_WFDE5(n, vegtype, fgrd, localmask)
   ...
   real              :: undef   ! input data fill value
   
   localmask(:,:) = LDT_rc%udef
   
#if (defined USE_NETCDF3 || defined USE_NETCDF4)
   
   call get_landmask(n,nlon,nlat,landmask_cap,undef)
   call interp_landmask(n, nlon, nlat, landmask_cap, localmask, undef)
   deallocate(landmask_cap)
   call check_landmask(n,vegtype,fgrd,localmask)
   write(LDT_logunit,*)'[INFO] Done processing WFDE5 land mask!'
   
#else
   
   write(LDT_logunit,*) &
        '[ERR] netCDF4 required to process WFDE5 land mask!'
   write(LDT_logunit,*) &
...
contains
   ...
   subroutine get_landmask(n, nlon, nlat, landmask_cap, undef)
      ...
      ! Get the variable ID
      status = nf90_inq_varid(ncid,"ASurf",varid)
      if (status .ne. NF90_NOERR) then
      ...
      status=nf90_get_att(ncid,varid,'_FillValue', undef)
   ...
   subroutine interp_landmask(n, nlon, nlat, landmask_cap, localmask, undef)
      ...
      case( "none", "mode", "neighbor", "tile")
         write(LDT_logunit,*) "[INFO] Reading WFDE5 land mask "
      case default
         write(LDT_logunit,*) &
              "[ERR] The spatial transform option selected for "
         write(LDT_logunit,*) &
              "   WFDE5 is not recognized nor recommended."
         write(LDT_logunit,*) &
              "   Please select: "
         write(LDT_logunit,*) &
...
end subroutine read_maskfile_WFDE5
