...
!=== Interpolate Data in time      
  wt1=(wfde5_struc(n)%wfde5time2-LIS_rc%time)/ & 
  ...
  mfactor = LIS_rc%nensem(n)/wfde5_struc(n)%nIter
...
! forward hour average
!        swd(t) = wfde5_struc(n)%metdata1(kk,3,index1)*zw1
! past hour average
        swd(t) = wfde5_struc(n)%metdata2(kk,3,index1)*zw1
...
!        kk = LIS_get_iteration_index(n, k, index1, mfactor)
!
!        if (wfde5_struc(n)%metdata1(kk,3,index1).ne.LIS_rc%udef.and.&
!             wfde5_struc(n)%metdata2(kk,3,index1).ne.LIS_rc%udef) then 
!           swd(t) = wfde5_struc(n)%metdata1(kk,3,index1)*zw1 + &
!                wfde5_struc(n)%metdata2(kk,3,index1)*zw2
!
!           ! In cases of small cos(zenith) angles, use linear weighting
!           !  to avoid overly large weights
!
!           if((swd(t).gt.wfde5_struc(n)%metdata1(kk,3,index1).and. & 
!                swd(t).gt.wfde5_struc(n)%metdata2(kk,3,index1)).and. & 
!                (czb.lt.0.1.or.cze.lt.0.1))then
!              swd(t) = wfde5_struc(n)%metdata1(kk,3,index1)*swt1+ & 
!                   wfde5_struc(n)%metdata2(kk,3,index1)*swt2
!           endif
...
!              write(unit=LIS_logunit,fmt=*)'[WARN] wfde5data1=',&
!                   wfde5_struc(n)%metdata1(kk,3,index1)
!              write(unit=LIS_logunit,fmt=*)'[WARN] wfde5data2=',&
!                   wfde5_struc(n)%metdata2(kk,3,index1)
...
!        endif
        ...
        if ((swd(t).ne.LIS_rc%udef).and.(swd(t).lt.0)) then
           if (swd(t).gt.-0.00001) then 
              ...
              write(LIS_logunit,*)'[ERR] timeinterp_wfde5 -- ', & 
                   t,swd(t),wfde5_struc(n)%metdata2(kk,3,index1), & 
                   ' (',LIS_localPet,')'
              call LIS_endrun
           endif
        endif
        ...
        if (swd(t).gt.LIS_CONST_SOLAR) then
           swd(t)=wfde5_struc(n)%metdata2(kk,3,index1)
!           swd(t)=wfde5_struc(n)%metdata1(kk,3,index1)
        endif
     enddo
  enddo
...
!-----------------------------------------------------------------------  
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        index1 = LIS_domain(n)%tile(t)%index
        kk = LIS_get_iteration_index(n, k, index1, mfactor)
!        pcp(t) = wfde5_struc(n)%metdata1(kk,7,index1)
        pcp(t) = wfde5_struc(n)%metdata2(kk,7,index1)
...
!-----------------------------------------------------------------------
  ...
  do k=1,LIS_rc%ntiles(n)/mfactor
     do m=1,mfactor
        ...
        tmp(t) = wfde5_struc(n)%metdata1(kk,1,index1)*wt1 + &
             wfde5_struc(n)%metdata2(kk,1,index1)*wt2
        q2(t)  = wfde5_struc(n)%metdata1(kk,2,index1)*wt1 + &
             wfde5_struc(n)%metdata2(kk,2,index1)*wt2
        lwd(t)  = wfde5_struc(n)%metdata1(kk,4,index1)*wt1 + &
             wfde5_struc(n)%metdata2(kk,4,index1)*wt2

        uwind(t) = wfde5_struc(n)%metdata1(kk,5,index1)*wt1+&
             wfde5_struc(n)%metdata2(kk,5,index1)*wt2
        vwind(t) = 0.0
        psurf(t) = wfde5_struc(n)%metdata1(kk,6,index1)*wt1 + &
             wfde5_struc(n)%metdata2(kk,6,index1)*wt2
