...
!EOP
  ...
  if(alarmCheck.or.(LIS_rc%endtime ==1)) then 
     if(LIS_masterproc) then 
        ...
        call LIS_verify(status,"Error in nf90_open in HYMAP3_routing_writerst")
#endif
     ...
     if (LIS_masterproc) then
        ...
        call LIS_verify(status, "Error in nf90_close in HYMAP3_routing_writerst")
#endif
     endif
...
!EOP 
    ...
    call HYMAP3_writeHeader_restart(ftn, n, dimID, bsfsto_ID, "BSFSTO", &
         "baseflow storage", &
         "-", 1, -99999.0, 99999.0)
    call HYMAP3_writeHeader_restart(ftn, n, dimID, rivout_pre_ID, "RIVOUT_PRE", &
         "rivoutpre", &
         "-", 1, -99999.0, 99999.0)
    call HYMAP3_writeHeader_restart(ftn, n, dimID, rivdph_pre_ID, "RIVDPH_PRE", &
         "river depth_pre", &
         "-", 1, -99999.0, 99999.0)
    call HYMAP3_writeHeader_restart(ftn, n, dimID, fldout_pre_ID, "FLDOUT_PRE", &
         "flood discharge pre", &
         "-", 1, -99999.0, 99999.0)
    call HYMAP3_writeHeader_restart(ftn, n, dimID, flddph_pre_ID, "FLDDPH_PRE", &
         "flood depth pre", &
         "-", 1, -99999.0, 99999.0)
!ag (27Apr2020)
    ...
    if(HYMAP3_routing_struc(n)%bifflag==1)then
      call HYMAP3_writeHeader_restart(ftn, n, dimID_bif, bifout_pre_ID, "BIFOUT_PRE", &
...
! 
! !DESCRIPTION: 
!    This routine writes the required NETCDF header for a single HYMAP3 variable
! 
!   The arguments are: 
!   \begin{description}
!   \item[n]
